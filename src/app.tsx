      ...
      <header className="bg-white shadow-md p-4 sm:p-6 border-b border-gray-200">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div className="mb-4 sm:mb-0">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 italic tracking-widest">🌤️ Weather App</h1>
            <p className="text-sm sm:text-base text-gray-600 mt-1">Current weather and 3-day forecast</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col md:flex-row max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="w-full md:w-64 md:sticky md:top-24 space-y-6 mb-6 md:mb-0">
          <div className="bg-white rounded-xl shadow p-4 border border-gray-100 hover:shadow-md transition-shadow">
            <h3 className="text-sm font-medium text-gray-700 mb-3">Search Weather</h3>
          ...
          {weather ? (
            <>
              ...
              <div className="bg-white rounded-xl shadow p-5 border border-gray-100 hover:shadow-md transition-shadow">
                <h2 className="text-lg font-semibold text-gray-700 mb-4">Current Weather in {weather.current.name}</h2>
                <div className="flex flex-col md:flex-row items-center justify-between">
                  <div className="flex items-center mb-4 md:mb-0">
                    ...
                    />
                    <div className="ml-4">
                      <p className="text-3xl font-bold text-gray-800">{Math.round(weather.current.main.temp)}°C</p>
                      <p className="text-gray-600">{weather.current.weather[0].description}</p>
                    </div>
                  </div>
                  <div className="text-center">
                    <p className="text-gray-600">Feels like {Math.round(weather.current.main.feels_like)}°C</p>
                    <p className="text-gray-600">Humidity: {weather.current.main.humidity}%</p>
                    <p className="text-gray-600">Wind: {weather.current.wind.speed} m/s</p>
                  </div>
                </div>
              </div>
              ...
              {/* 3-Day Forecast */}
              <div className="bg-white rounded-xl shadow p-5 border border-gray-100 hover:shadow-md transition-shadow">
                <h2 className="text-lg font-semibold text-gray-700 mb-4">3-Day Forecast</h2>
            ...
            <div className="bg-white rounded-xl shadow p-5 border border-gray-100 text-center">
              <p className="text-gray-500">Enter a location to get the weather</p>
            </div>
          )}
        </div>
      </main>
